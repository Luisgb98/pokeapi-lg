...
function InfoItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className="mt-1 text-sm font-semibold text-stone-700">{value}</dd>
    </div>
  );
}
...
export function SpeciesInfoSection({ species, labels }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm lg:col-span-2">
      ...
      {species.flavorText && (
        <blockquote className="relative mb-6 overflow-hidden rounded-xl bg-stone-50 px-5 py-4">
          <span
            aria-hidden="true"
            className="pointer-events-none absolute left-3 top-0 font-display text-6xl leading-none text-stone-200 select-none"
          >
            &ldquo;
          </span>
          <p className="relative pl-5 text-[0.9375rem] italic leading-relaxed text-stone-600">
            {species.flavorText}
          </p>
        </blockquote>
      )}
